    ...
    public class TwentyOneGame : Game, IWalkAway //this means class TwentyOneGame will inherit from class Game
    {
        ...
        public override void Play() //in order to inherit the abstract method, use the same name, data type, parameters, and the override keyword
        {
            ...
            foreach (Player player in Players)
            {
                ...
                if (!successfullyBet)
                {
                    return; //returning in a void method, so it just ends the method, and since in a while loop of activelyplaying and balance > 0 will hit the top of the play method again 
                }
                Bets[player] = bet; //Bets is the dictionary, player is the key, bet is the entry
            }
            for (int i = 0; i < 2; i++)
            {
            ...
            if (Dealer.isBusted)
            {
                ...
                foreach (KeyValuePair<Player, int> entry in Bets)
                {
                    ...
                    Players.Where(x => x.Name == entry.Key.Name).First().Balance += (entry.Value * 2); //vid.8 p.210
                    Dealer.Balance -= entry.Value;
                }
                return;
            }
            foreach (Player player in Players)
            {
                bool? playerWon = TwentyOneRules.CompareHands(player.Hand, Dealer.Hand); //now a nullable bool
                if (playerWon == null)
                {
